...
class CensusACSMedianIncomeETL(ExtractTransformLoad):
    ...
    def get_data_sources(self) -> [DataSource]:
        ...
        return [ZIPDataSource(source=self.GEOCORR_ALL_STATES_URL, destination=self.GEOCORR_ALL_STATES_PATH),
        FileDataSource(source=self.PUERTO_RICO_S3_LINK, destination=self.PUERTO_RICO_ALL_STATES_SOURCE),
        FileDataSource(source=self.MSA_MEDIAN_INCOME_URL, destination=self.MSA_MEDIAN_INCOME_SOURCE),
        FileDataSource(source=self.STATE_MEDIAN_INCOME_URL, destination=self.STATE_MEDIAN_INCOME_SOURCE),]
